...
The actual definition of the model that is bindable is provided through the decorators `@ViewModel(T)`. *T* is a type (class) that has properties decorated with validation and UI decorators. More about this can be found [here](vm.md).
...
Binding Handlers are small functions calls that handle the data flow between viewmodel property and element attribute. There a few [default binding handlers available](nbind.md#standard-binding-handlers).

#### Smart Binders

Instead of using the string form you can use the JSX syntax and binding functions:
...
See [here](nbind.md#smart-binders) for all details.
...
The forms module comes with a couple of pre-defined bindeing handlers:
...
1. The viewmodel type. Mandatory.
2. The element type. This is optional, if omitted it falls back to `HTMLElement`.

The parameters are as follows:

1. A lambda expression to select a property type safe (`c => c.name`). Mandatory.
...
additional editor support. To overcome the limitations here, the `bind` function is available.
