 ...
 */
export function InjectService<T extends {} | Constructor<T> = any>(name: string, type: ServiceType<T> | T, singleton: boolean = false) {
  return function (target: any) {
    ...
    if (!(<Map<string, any>>target.prototype[InjectServices_Symbol]).has(name)) {
      const t = singleton ? type : new (type as Constructor<T>)();
