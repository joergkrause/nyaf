...
export function contains(s: string): boolean {
...
export function toArray(delimiter = ',', type = 'string', format = false) {
    let a;
...
export function dashedName() {
    return this.replace(/([A-Z])/g, function(u) { return '-' + u.toLowerCase(); });
}

export function isDate(format, options) {
    let result = '';
